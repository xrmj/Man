...
var adminClient =function () {
    this.host = '116.31.99.217';//window.location.hostname;
    this.port = 3014;
    this.httpHost = location.href.replace(location.hash, '');
    this.httpHost = "http://116.31.99.217:3000/";//this.httpHost.substr(0, this.httpHost.lastIndexOf('/') + 1);
};
...
adminClient.prototype.playerCtrl = function(uid, ctrl, cb) {
	var self = this;
	pomelo.request('connector.entryHandler.playerCtrl', {
		uid : uid,
		ctrl : ctrl
	}, function(res) {
		if (!res.result || res.result.code != 200) {
